...
export function calculateSmokingpipesReferencePrice({
  ...
  const serviceFeeRate = safeRate(pricingConfig?.serviceFeeRate, 0.15);
  const minServiceFeeCny = safeRate(pricingConfig?.minServiceFeeCny, 200);
  const taxableProductCostCny =
    purchasePriceUsd * importCostFactor * usdToCny;
  const shippingCny = shippingUsd * usdToCny;
  const baseCostCny = taxableProductCostCny + shippingCny;
