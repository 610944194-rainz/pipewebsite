...
export function calculateSmokingpipesReferencePrice({
  ...
  const taxFactor = safeRate(pricingConfig?.taxFactor, 1);
  ...
  const shippingUsd = safeRate(tier.shippingUsd, 0);
  const taxableProductCostCny = purchasePriceUsd * usdToCny * taxFactor;
  const shippingCny = shippingUsd * usdToCny;
  const baseCostCny = taxableProductCostCny + shippingCny;
