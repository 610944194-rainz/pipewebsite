...
export function calculateReferencePrice({
  ...
  pricingConfig = REFERENCE_PRICE_COMMON_CONFIG,
}) {
  if (
    !isPositiveFiniteNumber(sourcePriceAmount) ||
    !isPositiveFiniteNumber(sourceToCny) ||
    !isNonNegativeFiniteNumber(internationalShippingAmount)
  ) {
    return {
      siteDisplayReady: false,
      siteDisplayAmount: null,
      siteDisplayCurrency: null,
      importCostFactor: null,
  ...
  const importCostFactor = safePositiveRate(
    pricingConfig?.importCostFactor ?? pricingConfig?.taxFactor,
    REFERENCE_PRICE_COMMON_CONFIG.taxFactor
  );
  const serviceFeeRate = safeRate(
    pricingConfig?.serviceFeeRate,
    REFERENCE_PRICE_COMMON_CONFIG.serviceFeeRate
  );
  const minServiceFeeCny = safeRate(
    pricingConfig?.minServiceFeeCny,
    REFERENCE_PRICE_COMMON_CONFIG.minServiceFeeCny
  );
  const taxableProductCostCny = sourcePriceAmount * importCostFactor * sourceToCny;
  const shippingCny = internationalShippingAmount * sourceToCny;
...
export function calculateDanishPipeShopReferencePrice({
  ...
  return calculateReferencePrice({
    sourcePriceAmount: netExportPriceUsd,
    sourceToCny: usdToCny,
    internationalShippingAmount: shippingUsd,
    pricingConfig,
  }).siteDisplayAmount;
}
...
function safePositiveRate(value, fallback) {
...
export function calculateSmokingpipesReferencePrice({
  ...
  const reference = calculateReferencePrice({
    sourcePriceAmount: purchasePriceUsd,
    sourceToCny: usdToCny,
    internationalShippingAmount: shippingUsd,
    pricingConfig,
  });
