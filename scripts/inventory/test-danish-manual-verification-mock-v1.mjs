...
console.log("mock: V18 bridge sends the configured ShadowBot hotkey");
{
  ...
  assert.match(helper, /SendHotkey\(bool control, bool shift, bool alt, int virtualKey\)/);
  assert.match(helper, /ValidateHotkey\(bool control, bool shift, bool alt, int virtualKey\)/);
  assert.doesNotMatch(helper, /\[ushort\[\]\]/);
  assert.match(helper, /TestOnly/);
  assert.match(helper, /releaseIndex=inputs\.Length-1-i/);
  assert.match(helper, /keys\.Add\(\(ushort\)0x11\)/);
  assert.match(helper, /keys\.Add\(\(ushort\)0x10\)/);
  assert.match(helper, /keys\.Add\(\(ushort\)0x12\)/);
